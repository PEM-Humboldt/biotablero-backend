# Biotablero Ecosystems Service

Ecosystems service for biotablero
...
To build the image run: `docker build -t biotablero-ecosystems:<version> .`
...
**NOTE**: _If you change the server port in the configuration (4004 by default), you'll need to change the exposed port in the Dockerfile and rebuild the image._
...
```
docker run --restart always --name biotablero-ecosystems -p 4004:4004 -v <path/to/your/production.json>:/home/node/app/config/production.json -e NODE_CONFIG_ENV=production -d biotablero-ecosystems:<version>
```
